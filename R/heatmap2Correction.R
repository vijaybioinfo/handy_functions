...
)
{
  ...
  if( missing(lmat) || is.null(lmat) )
  {
    ...
    if(!is.null(ColSideColors)) { ## add middle row to layout
    ...
    if(!is.null(RowSideColors)) { ## add middle column to layout
...
invalid<-function(x=NA){
  if(sum(is.na(x))>0 || sum(is.null(x))>0){
    return(TRUE)
  }else{
    return(FALSE)
  }
